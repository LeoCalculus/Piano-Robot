...
class PianoApp:
    def __init__(self, root: tk.Tk):
        ...
        ctrl.pack(pady=(6, 12))
        bcfg = dict(font=('Courier', 10), bg='#1e293b', fg='#94a3b8', relief='flat', padx=12, pady=4, cursor='hand2')
    ...
    def _overlap_warnings(self, s: Step) -> List[str]:
        ...
        for i in range(len(active)):
            for j in range(i+1, len(active)):
                la, ca, ha = active[i]; lb, cb, hb = active[j]
                if max(ca-ha, cb-hb) < min(ca+ha, cb+hb): issues.append(f'{la}↔{lb}')
        return issues
    ...
    def _play(self):
        if self._song: self._stop(); self._step(0)
    ...
    def _step(self, i):
        if i >= len(self._song): self.info.set("Done."); return
