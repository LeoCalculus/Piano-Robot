...
Initial calibration (lpos=0 / rpos=0 mechanical zero):
  ...
  Right F2 → F6 centre  (72.45 cm), i.e. rpos=0 means hand at right end
...
RPOS_HOME = 67.85      # right F0 → D6, so F2 → F6 (72.45 cm)
...
RIGHT_FINGERS: List = [
    ('W', 0.0),   # F0 anchor (= D6 at home)
    ('B', 1.4),   # F1 → D#6 at home  (D6→D#6 centre = 1.4 cm)
    ('W', 4.6),   # F2 → F6 at home  (D6→F6 centre = 4.6 cm)
]
...
class PianoApp:
    ...
    def _render_hands(self, lpos: float, rpos: float,
        ...
        ox = PAD
        lx = ox + lpos * PX
        rx = ox + (rpos + 4.6) * PX   # F2 of right hand
        self.hcv.create_text(lx, 1, text=f"L:{lpos:.1f}", fill='#60a5fa',
                             font=('Courier', 7), anchor='n')
        self.hcv.create_text(rx, 1, text=f"R:{rpos+4.6:.1f}", fill='#34d399',
                             font=('Courier', 7), anchor='n')
    ...
    def _step(self, i: int):
        ...
        self.info.set(f"Moving → step {i+1}/{n_total}  "
                      f"L:{s.lpos:.1f} cm  R(F2):{s.rpos+4.6:.1f} cm")
    ...
    def _on_settled(self):
        ...
        self.info.set(
            f"Step {i+1}/{n_total}  "
            f"L:{s.lpos:.1f}  R(F2):{s.rpos+4.6:.1f}  "
            f"sol={''.join(str(x) for x in s.solenoids)}  "
            f"{s.duration} ms{flags}"
        )

        if warnings:
            self.warn.set("⚠ OUT OF SCOPE: " + "  ".join(
