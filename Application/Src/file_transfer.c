/*
 * file_transfer.c - MIDI file transfer via Bluetooth
 * Implements chunked binary protocol for reliable file upload
 */
...
static int FT_FlushWriteBuffer(void) {
    if (ftWriteBufferIndex == 0) return 0;
...
static int FT_HandleFileStart(uint8_t* packet, uint16_t length) {
    ...
    /* Check if file already exists - delete if so (overwrite mode) */
    FIL testFile;
    if (f_open(&testFile, ftContext.filename, FA_READ) == FR_OK) {
        f_close(&testFile);
        f_unlink(ftContext.filename);  /* Delete existing file */
    }
    ...
    if (res != FR_OK) {
        uint8_t errCode = (res == FR_DISK_ERR) ? FT_ERR_SD_WRITE : FT_ERR_SD_FULL;
        FT_SendResponse(FT_RSP_ERROR, &errCode, 1);
        return -1;
    }
